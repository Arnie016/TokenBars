...
def main() -> int:
    ...
    try:
        ...
        require("tokenbar trailer" in help_output.stdout, "help output missing identity trailer command")
        require("tokenbar trailer --html" in help_output.stdout, "help output missing identity trailer HTML export command")
        ...
        video_output = run([str(tokenbar_cli), "trailer"], publish_env, timeout=90)
        require("HyperFrames-ready" in video_output.stdout, "video output missing HyperFrames direction")
        require("Director Bay" in video_output.stdout, "video output missing Director Bay bridge")
        ...
        require("30-second micro-storyboard" in video_output.stdout, "video output missing micro-storyboard")
        video_json = run([str(tokenbar_cli), "trailer", "--json"], publish_env, timeout=90)
        ...
        require(video_payload.get("kind") == "identityTrailer", "video JSON missing identity trailer kind")
        ...
        require(len(video_payload.get("voiceTimingPlan") or []) == 5, "video JSON missing voice timing contract")
        require(video_payload.get("directorBayBridge", {}).get("sourceThread") == "019e7a08-b6d6-7863-be79-66b20c9353df", "video JSON missing AI video editor source thread")
        require(video_payload.get("promptEditableManifest", {}).get("fieldGroups"), "video JSON missing prompt-editable manifest")
        require(video_payload.get("interactionEvents"), "video JSON missing interaction events")
        ...
        trailer_html = run([str(tokenbar_cli), "trailer", "--html"], publish_env, timeout=90)
        html_match = re.search(r"storyboard:\s*(.+\.html)", trailer_html.stdout)
        require(html_match, "trailer HTML output missing storyboard path")
        trailer_html_path = Path(html_match.group(1).strip())
        require(trailer_html_path.exists(), "trailer HTML storyboard file was not written")
        trailer_html_text = trailer_html_path.read_text(encoding="utf-8")
        require("TokenBar Builder Identity Trailer" in trailer_html_text, "trailer HTML missing title")
        require("Your AI work becomes a proof story." in trailer_html_text, "trailer HTML missing hero")
        require("voiceTimingPlan" in trailer_html_text, "trailer HTML missing embedded manifest")
        require("raw transcripts" in trailer_html_text and "source code" in trailer_html_text, "trailer HTML missing privacy boundary")
